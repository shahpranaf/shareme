...
const CreatePin = ({user}) => {
  ...
  const uploadImage = (e) => {
    ...
    if (imgTypeArr.includes(selectedFile.type)) {
      setWrongImageType(false);
      setImageAsset(URL.createObjectURL(selectedFile));
      setImgUploadObj(selectedFile);
    } else {
  ...
  const savePin = async () => {
    ...
    try {
      if (title && about && destination && imageAsset && category) {
        const document = await client.assets.upload('image', imgUploadObj, { contentType: imgUploadObj.type, filename: imgUploadObj.name });
